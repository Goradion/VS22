...
int ServerComWebserviceImplPortBindingProxy::receiveSoapableMessage(const char *endpoint, const char *soap_action, ns1__receiveSoapableMessage *ns1__receiveSoapableMessage_, ns1__receiveSoapableMessageResponse &ns1__receiveSoapableMessageResponse_)
{	struct soap *soap = this->soap;
	...
	if (soap_endpoint == NULL)
		soap_endpoint = "http://localhost:8080/TafelWS/serverws";
	if (soap_action == NULL)
		soap_action = "http://webservice.tafelServer/ServerComWebservice/receiveSoapableMessageRequest";
	soap_tmp___ns1__receiveSoapableMessage.ns1__receiveSoapableMessage_ = ns1__receiveSoapableMessage_;
	soap_begin(soap);
	soap->encodingStyle = NULL;
	soap_serializeheader(soap);
	soap_serialize___ns1__receiveSoapableMessage(soap, &soap_tmp___ns1__receiveSoapableMessage);
	if (soap_begin_count(soap))
		return soap->error;
	if (soap->mode & SOAP_IO_LENGTH)
	{	if (soap_envelope_begin_out(soap)
		 || soap_putheader(soap)
		 || soap_body_begin_out(soap)
		 || soap_put___ns1__receiveSoapableMessage(soap, &soap_tmp___ns1__receiveSoapableMessage, "-ns1:receiveSoapableMessage", "")
		 || soap_body_end_out(soap)
		 || soap_envelope_end_out(soap))
			 return soap->error;
	}
	...
	if (soap_connect(soap, soap_endpoint, soap_action)
	 ...
	 || soap_end_send(soap))
		return soap_closesock(soap);
	if (!static_cast<ns1__receiveSoapableMessageResponse*>(&ns1__receiveSoapableMessageResponse_)) // NULL ref?
		return soap_closesock(soap);
	...
	if (soap_begin_recv(soap)
	 ...
	 || soap_body_begin_in(soap))
		return soap_closesock(soap);
	ns1__receiveSoapableMessageResponse_.soap_get(soap, "ns1:receiveSoapableMessageResponse", NULL);
	if (soap->error)
		return soap_recv_fault(soap, 0);
	if (soap_body_end_in(soap)
	 || soap_envelope_end_in(soap)
